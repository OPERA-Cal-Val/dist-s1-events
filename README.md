...
There will be additional sources used to derive forthecoming sites. For now, all the sites in this repository (i.e. in `events/`) are derived from the above 3 sources. We note that all the datasets are delineated using *optical* sensors (either Sentinel-1, Landsat, planet or other VHR sensors and the exact provenance of each dataset can be traced using the source data). These optically-derived delineations are valuable datasets for calibrating/validating SAR disturbances as some aspects of the event will not be visible in SAR (e.g. a brush fire typically is not visible in SAR imagery but can be in optical data). We highlight that validating any imagery across sensors can be impacted by the differences in acquisition time, particularly when imaging dynamic events like floods. In other words, care must be used for *each* event!
